...
const splitJournalEntryIntoPages = (entryText, pageCount = 1, linesPerPage = 22) => {
  const source = String(entryText || '');
  const pages = Array.from({ length: Math.max(1, pageCount) }, () => '');
  if (!source.trim()) return pages;

  const lines = source.split('\n');
  const chunkSize = Math.max(1, linesPerPage);

  for (let index = 0; index < pages.length; index += 1) {
    const start = index * chunkSize;
    const end = start + chunkSize;
    pages[index] = lines.slice(start, end).join('\n');
  }

  if (lines.length > pages.length * chunkSize) {
    const overflow = lines.slice(pages.length * chunkSize).join('\n');
    pages[pages.length - 1] = [pages[pages.length - 1], overflow].filter(Boolean).join('\n');
  }

  return pages;
};

const NOTEBOOK_LINE_HEIGHT_PX = 34;
const NOTEBOOK_RULE_OFFSET_PX = 24;
const NOTEBOOK_TEXT_PADDING_TOP_PX = 12;
const NOTEBOOK_RULE_COLOR = 'rgba(237,240,244,0.07)';

const getNotebookRuleStyle = () => ({
  backgroundImage: `linear-gradient(to bottom, transparent ${NOTEBOOK_RULE_OFFSET_PX}px, ${NOTEBOOK_RULE_COLOR} ${NOTEBOOK_RULE_OFFSET_PX}px, ${NOTEBOOK_RULE_COLOR} ${NOTEBOOK_RULE_OFFSET_PX + 1}px, transparent ${NOTEBOOK_RULE_OFFSET_PX + 1}px)`,
  backgroundSize: `100% ${NOTEBOOK_LINE_HEIGHT_PX}px`,
  backgroundRepeat: 'repeat-y'
});

const normalizeMarkdownForNotebook = (markdownText = '') => {
...
const parseMarkdown = (markdown) => {
  ...
  return escapedMarkdown
    // Headers
    .replace(/^### (.*$)/gim, '<h3 class="text-[1.2rem] font-semibold text-white mb-3 mt-6">$1</h3>')
    .replace(/^## (.*$)/gim, '<h2 class="text-[1.35rem] font-semibold text-white mb-4 mt-8">$1</h2>')
    .replace(/^# (.*$)/gim, '<h1 class="text-[1.9rem] font-bold text-white mb-6 mt-8">$1</h1>')
...
const NOTEBOOK_SECTION_TITLES = new Set([
  'Overview',
  'Activities',
  'Topics',
  'Mindmaps',
...
const getNotebookLineDisplayMeta = (line = '', index = 0) => {
  ...
  if (NOTEBOOK_SECTION_TITLES.has(trimmed)) {
    return {
      displayText: value,
  ...
  if (/^###\s+/.test(trimmed)) {
    return {
      displayText: value.replace(/^\s*###\s+/, ''),
      className: 'text-[19px] font-semibold text-gray-100'
    };
  }
  ...
  return {
    displayText: value,
    className: 'text-[15px] text-gray-100'
  };
};

const NotebookPageContent = ({ text }) => {
  const lines = String(text || '').split('\n');

  return (
    <div className="relative z-10 text-sm">
      {lines.map((line, index) => {
        const { displayText, className } = getNotebookLineDisplayMeta(line, index);
        return (
          <div
            key={`notebook_line_${index + 1}`}
            className={`whitespace-pre-wrap break-words ${className}`}
            style={{
              lineHeight: `${NOTEBOOK_LINE_HEIGHT_PX}px`,
              minHeight: `${NOTEBOOK_LINE_HEIGHT_PX}px`
            }}
          >
            {displayText || '\u00A0'}
          </div>
...
const Journal = () => {
  ...
  const [journalPageMode, setJournalPageMode] = useState('single');
  ...
  useEffect(() => {
    const savedPageMode = localStorage.getItem(getUserStorageKey('journalPageMode'));
    if (savedPageMode === 'single' || savedPageMode === 'dual') {
      setJournalPageMode(savedPageMode);
      return;
    }

    setJournalPageMode('single');
  }, [userStorageId]);

  useEffect(() => {
    if (journalPageMode !== 'single' && journalPageMode !== 'dual') return;
    localStorage.setItem(getUserStorageKey('journalPageMode'), journalPageMode);
  }, [journalPageMode, userStorageId]);
  ...
  const notebookPageCount = !isPhoneViewport && journalPageMode === 'dual' ? 2 : 1;
  const notebookDisplayText = useMemo(() => {
    if (journalSettings.journalFormat === 'markdown') {
      return normalizeMarkdownForNotebook(currentEntry);
    }
    return currentEntry;
  }, [currentEntry, journalSettings.journalFormat]);
  const notebookPages = useMemo(
    () => splitJournalEntryIntoPages(notebookDisplayText, notebookPageCount),
    [notebookDisplayText, notebookPageCount]
  );
  const notebookEditablePages = useMemo(
    () => splitJournalEntryIntoPages(currentEntry, notebookPageCount),
    [currentEntry, notebookPageCount]
  );

  const updateNotebookEditPage = (pageIndex, pageText) => {
    if (notebookPageCount === 1) {
      setCurrentEntry(pageText);
      setIsEditing(true);
      return;
    }

    const updatedPages = [...notebookEditablePages];
    updatedPages[pageIndex] = pageText;
    setCurrentEntry(updatedPages.join('\n'));
    setIsEditing(true);
  };
  ...
  return (
    <div className="bg-black text-white min-h-screen flex">
      ...
      <div className={`flex-1 flex flex-col transition-[margin] duration-300 ${
        ...
        {showSettings && (
          <div
            className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto scrollbar-themed bg-black/70 px-4 py-8"
            onMouseDown={(event) => {
          ...
          >
            <div className="w-full max-w-4xl overflow-hidden rounded-2xl border border-white/15 bg-black shadow-2xl max-h-[85vh] flex flex-col">
              <div className="flex items-start justify-between gap-4 border-b border-white/10 p-5">
                <div>
                  <h3 className="text-lg font-semibold text-white">Journal Settings</h3>
                  <p className="text-sm text-gray-400">
                    Configure auto journal, templates, and GitHub sync preferences.
                  </p>
                </div>
                <button
              ...
              <div className="overflow-y-auto scrollbar-themed p-5">
                  ...
                  {loading || !initialLoadComplete ? (
                    ...
                    <div className="space-y-4">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="inline-flex items-center rounded-lg border border-white/15 bg-black/65 p-1">
                            <button
                              onClick={() => setJournalPageMode('single')}
                              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${journalPageMode === 'single' ? 'bg-emerald-500/25 text-emerald-100' : 'text-gray-300 hover:bg-white/5'}`}
                            >
                              Single Page
                            </button>
                            <button
                              onClick={() => setJournalPageMode('dual')}
                              className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${journalPageMode === 'dual' ? 'bg-emerald-500/25 text-emerald-100' : 'text-gray-300 hover:bg-white/5'}`}
                            >
                              Dual Page
                            </button>
                          </div>
                      ...
                      <div className="rounded-xl border border-white/15 bg-[#090909] p-3 sm:p-4">
                        <div className={`relative ${notebookPageCount === 2 ? 'grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6' : ''}`}>
                          {notebookPageCount === 2 ? (
                            <div className="hidden lg:flex pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 h-full w-16 items-center justify-center">
                              <div className="absolute h-full w-px bg-white/15" />
                              <div className="flex h-full flex-col items-center justify-evenly">
                                {Array.from({ length: 8 }).map((_, ringIndex) => (
                                  <span
                                    key={`journal_edit_ring_${ringIndex + 1}`}
                                    className="h-4 w-8 rounded-full border border-white/65 bg-black/90"
                                  />
                                ))}
                              </div>
                            </div>
                          ...
                          <div className={`relative rounded-lg border border-emerald-300/45 bg-black px-6 py-5 ${notebookPageCount === 2 ? 'min-h-[28rem]' : 'min-h-[36rem]'} shadow-[0_8px_24px_rgba(0,0,0,0.35)]`}>
                            <div className="pointer-events-none absolute inset-0 rounded-lg" style={getNotebookRuleStyle()} />

                            <textarea
                              value={notebookPageCount === 2 ? (notebookEditablePages[0] || '') : currentEntry}
                              onChange={(e) => {
                                updateNotebookEditPage(0, e.target.value);
                              }}
                              onFocus={() => setIsEditing(true)}
                              placeholder="Write your journal entry here... You can use markdown formatting."
                              className={`relative z-10 w-full ${notebookPageCount === 2 ? 'h-[28rem]' : 'h-[36rem]'} bg-transparent text-sm text-gray-100 placeholder-gray-500 resize-none focus:outline-none`}
                              style={{
                                lineHeight: `${NOTEBOOK_LINE_HEIGHT_PX}px`,
                                paddingTop: `${NOTEBOOK_TEXT_PADDING_TOP_PX}px`,
                                paddingBottom: 0,
                                paddingLeft: '2px',
                                paddingRight: '2px',
                                margin: 0
                          ...
                          {notebookPageCount === 2 ? (
                            <div className="hidden lg:block relative rounded-lg border border-emerald-300/45 bg-black px-6 py-5 min-h-[28rem] shadow-[0_8px_24px_rgba(0,0,0,0.35)]">
                              <div className="pointer-events-none absolute inset-0 rounded-lg" style={getNotebookRuleStyle()} />
                              <textarea
                                value={notebookEditablePages[1] || ''}
                                onChange={(e) => {
                                  updateNotebookEditPage(1, e.target.value);
                                }}
                                onFocus={() => setIsEditing(true)}
                                placeholder="Continue on the right page..."
                                className="relative z-10 w-full h-[28rem] bg-transparent text-sm text-gray-100 placeholder-gray-500 resize-none focus:outline-none"
                                style={{
                                  lineHeight: `${NOTEBOOK_LINE_HEIGHT_PX}px`,
                                  paddingTop: `${NOTEBOOK_TEXT_PADDING_TOP_PX}px`,
                                  paddingBottom: 0,
                      ...
                      {currentEntry ? (
                        <div className="rounded-xl border border-white/15 bg-[#090909] p-3 sm:p-4">
                          <div className={`relative ${notebookPageCount === 2 ? 'grid grid-cols-1 lg:grid-cols-2 gap-4 lg:gap-6' : ''}`}>
                            {notebookPageCount === 2 ? (
                              <div className="hidden lg:flex pointer-events-none absolute left-1/2 top-0 -translate-x-1/2 h-full w-16 items-center justify-center">
                                ...
                                <div className="flex h-full flex-col items-center justify-evenly">
                                  {Array.from({ length: 8 }).map((_, ringIndex) => (
                                    <span
                                      key={`journal_view_ring_${ringIndex + 1}`}
                                      className="h-4 w-8 rounded-full border border-white/65 bg-black/90"
                                    />
                                  ))}
                                </div>
                            ...
                            {Array.from({ length: notebookPageCount }).map((_, pageIndex) => (
                              <div
                                key={`journal_page_${pageIndex + 1}`}
                                className={`relative rounded-lg border border-emerald-300/45 bg-black px-6 py-5 min-h-[28rem] shadow-[0_8px_24px_rgba(0,0,0,0.35)] ${notebookPageCount === 2 ? 'hidden lg:block first:block' : ''}`}
                              >
                                <div
                                  className="pointer-events-none absolute inset-0 rounded-lg"
                                  style={getNotebookRuleStyle()}
                                />
                                ...
                                >
                                  <NotebookPageContent text={notebookPages[pageIndex] || ''} />
                                </div>
                              </div>
                            ))}
                          </div>
