...
const SIM_MARKER = 'sim-cse-mar2026-v2';
const EMAIL = 'arun@gmail.com';
const START_DATE = '2026-03-05';
const END_DATE = '2026-04-05';
const TARGET_TOPICS = 112;
const RNG_SEED = 20260405;
...
const getOrCreateUser = async () => {
  ...
  if (user) return { user, wasCreated: false };

  const username = await ensureUniqueUsername('arun_cse26');

  user = await User.create({
    username,
    email: EMAIL,
    password: 'ArunStudy#2026',
    memScore: 9,
    ...
    evaluationResults: {
      ...
      processingSpeed: 88,
      overallScore: 90,
      completedAt: new Date('2026-03-04T19:20:00')
    },
    preferences: {
    ...
    longestStreak: 14,
    totalStudyDays: 26,
    lastStudyDate: new Date('2026-04-05T21:05:00'),
    lastMemScoreUpdate: new Date('2026-04-05T20:00:00')
  });
...
const cleanupOldSimulationData = async (userId) => {
  const markers = ['sim-cse-mar2026-v1', 'sim-cse-mar2026-v2'];
  ...
  const revisionFilter = {
    userId,
    $or: [{ notes: /sim-cse-mar2026-v[12]/i }]
  };
  ...
  creationPlan.forEach((entry) => {
    ...
    for (let index = 0; index < entry.count; index += 1) {
      ...
      const doc = {
        ...
        estimatedMinutes: randomInt(22, 65),
...
const buildMindmaps = (states, dates) => {
  const activeTopics = shuffle(Array.from(states.values()).filter((item) => item.isActive)).slice(0, 8);
  const pickDates = shuffle(dates.slice(5, Math.max(6, dates.length - 2))).slice(0, 8);
  const mindmaps = [];
...
const buildChronicleEvents = ({ dates, revisionsByDate, hardSkipDay, docsCreatedByDate, mindmapsByDate }) => {
  const grouped = {};
      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            content,
            mood,
            tags: [SIM_MARKER, 'cse-btech', 'monthly-simulation'],
            activities,
...
const buildLocalStoragePayload = ({ userId, tasks, focusSessions, mindmaps, chronicleEvents }) => {
  return {
...
async function main() {
  ...
  const endDate = endOfDay(parseYmd(END_DATE));
  ...
  try {
    ...
    await MemScoreHistory.create({
      ...
      createdAt: new Date('2026-04-05T20:00:00'),
      updatedAt: new Date('2026-04-05T20:00:00')
    });
    ...
    const seedJsonPath = path.join(outputDir, 'arun_frontend_seed.json');
    fs.writeFileSync(seedJsonPath, JSON.stringify({
      metadata: {
        marker: SIM_MARKER,
        generatedAt: new Date().toISOString(),
        range: { start: START_DATE, end: END_DATE },
        note: 'Apply localStorage payload after login as Arun.'
      },
      user: {
        id: String(user._id),
        email: user.email,
    ...
    const applySnippet = [
      '// Paste in browser console while logged in as Arun',
      ...
      "console.log('Arun simulation localStorage applied.');"
    ].join('\n');

    const snippetPath = path.join(outputDir, 'arun_frontend_apply_snippet.js');
