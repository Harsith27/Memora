...
const SIMULATION_DAYS = Math.max(7, Number(process.env.SIM_DAYS) || 90);
const SIM_MARKER = process.env.SIM_MARKER || 'sim-lucky-cs-30d-v1';
const EMAIL = (process.env.SIM_USER_EMAIL || 'Lucky@gmail.com').trim();
const PASSWORD = process.env.SIM_USER_PASSWORD || 'Lucky@123';
const DISPLAY_NAME = process.env.SIM_DISPLAY_NAME || 'Lucky';
const USERNAME_BASE = (process.env.SIM_USERNAME || EMAIL.split('@')[0]).toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '') || 'lucky';
...
const TARGET_TOPICS = Math.max(45, Math.round(SIMULATION_DAYS * 3.5));
      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            content,
            mood,
            tags: [SIM_MARKER, 'cse-btech', 'ninety-day-simulation'],
            activities,
