...
const SIMULATION_DAYS = Math.max(30, Number(process.env.SIM_DAYS) || 50);
const SIM_MARKER = process.env.SIM_MARKER || 'sim-veera-cs-50d-v1';
const EMAIL = (process.env.SIM_USER_EMAIL || 'veera@gmail.com').trim();
const PASSWORD = process.env.SIM_USER_PASSWORD || 'Veera@123';
const DISPLAY_NAME = process.env.SIM_DISPLAY_NAME || 'Veera';
const USERNAME_BASE = (process.env.SIM_USERNAME || EMAIL.split('@')[0]).toLowerCase().replace(/[^a-z0-9_]+/g, '_').replace(/^_+|_+$/g, '') || 'veera';
